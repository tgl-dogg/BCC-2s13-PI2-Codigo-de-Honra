...
static int img_size = 5;
static ALLEGRO_BITMAP *im_prog_set[5];
static ALLEGRO_BITMAP *im_cond_set[5];
static ALLEGRO_BITMAP *im_act_set[5];
...
int create_desafio(challenger_rule cr){
    ...
    im_prog_set[3] = im_prog_set[4] = NULL;
...
void click_prog_buttons(clk_flag flags, challenger_rule cr, card_pile *cp){
	...
	if(flags.ev_status == 1){
        ...
        if(flags.card_pos < cr.prog){
            ...
            // a nossa definição de index de cartas.
